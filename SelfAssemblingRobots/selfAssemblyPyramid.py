# Self Assembly of a two sided pyramid 
...
formationCountD = 0;
formationCountG = 0;
formationClayerAbove = 0;
formationDlayerAbove = 0;
aveKECounter = 0;
aveaveKE = 0;
bodies = []
geoms = []
...
def computeForces():
    ...
    for b in bodies:
        ...
        # Add an impulse component
        # xCurr1 = random.gauss(0.0,3.0)
        # yCurr1 = random.gauss(0.0,3.0)
        # zCurr1 = random.gauss(0.0,3.0)
        # xCurrent1,yCurrent1,zCurrent1 = b.vectorFromWorld((xCurr1,yCurr1,zCurr1));
        # xdrag1 = -0.5*1000*ly*lz*1.05*(xCurrent1)*math.fabs(xCurrent1)
        # ydrag1 = -0.5*1000*lx*lz*1.05*(yCurrent1)*math.fabs(yCurrent1)
        # zdrag1 = -0.5*1000*lx*ly*1.05*(zCurrent1)*math.fabs(zCurrent1)
        # b.addRelForce((xdrag1,ydrag1,zdrag1))
        # xVel1,yVel1,zVel1 = b.getLinearVel();
        # xCurrent1,yCurrent1,zCurrent1 = b.vectorFromWorld((xVel1,yVel1,zVel1));
        

        # xdrag1 = -0.5*1000*ly*lz*1.05*(xCurrent1)*math.fabs(xCurrent1)
        # ydrag1 = -0.5*1000*lx*lz*1.05*(yCurrent1)*math.fabs(yCurrent1)
        # zdrag1 = -0.5*1000*lx*ly*1.05*(zCurrent1)*math.fabs(zCurrent1)
        # b.addForce((xdrag1,ydrag1,zdrag1));
        
        xCurr1 = random.uniform(-5.0,5.0)
        yCurr1 = random.uniform(-5.0,5.0)
        zCurr1 = random.uniform(-5.0,5.0)

        # xKe,yKe,zKe = b.getLinearVel();
        # createVec = [xKe,yKe,zKe];
        # velVec = length(createVec);
        # KE = 0.5*lx*ly*lz*1000*velVec*velVec;
        # aveKE = aveKE + KE;

        # if b.id == objcount:
        #     aveKE = aveKE/objcount
        #     aveaveKE = aveaveKE + aveKE;
        #     aveKECounter = aveKECounter + 1;

        # if aveKECounter == 1000:
        #     aveaveKE = aveaveKE/1000;
        #     print aveaveKE;
        #     aveKECounter = 0;
        #     aveaveKE = 0;
        ...
        # xCurrent1,yCurrent1,zCurrent1 = b.vectorFromWorld((xCurr1,yCurr1,zCurr1));
        # xCurrent1,yCurrent1,zCurrent1 = b.vectorFromWorld((xVel1-xCurr1,yVel1-yCurr1,zVel1-zCurr1));
        # xdrag = -0.5*1000*ly*lz*1.05*(xCurrent1)*math.fabs(xCurrent1)
        # ydrag = -0.5*1000*lx*lz*1.05*(yCurrent1)*math.fabs(yCurrent1)
        # zdrag = -0.5*1000*lx*ly*1.05*(zCurrent1)*math.fabs(zCurrent1)
        # b.addRelForceAtRelPos((xdrag,ydrag,zdrag),(0,0,0));


        # xCurr1 = random.uniform(-3.0,3.0)
        # yCurr1 = random.uniform(-3.0,3.0)
        # zCurr1 = random.uniform(-3.0,3.0)
        ...
        for i in range(0,8):
            ...
            # xCurr1 = (a0 + rC1)*math.cos(2*math.pi*f0*xpos) + 0*(a0 + rC1)*math.sin(2*math.pi*f0*xpos + randomComponent1) + (a2 + rC2)*math.sin(2*math.pi*f2*nowTime) + (a2 + rC2)*math.sin(2*math.pi*f2*nowTime + randomComponent2);
            # yCurr1 = a0*math.sin(2*math.pi*f0*ypos) + a1*math.cos(2*math.pi*f1*ypos) + a2*math.sin(2*math.pi*4*f2*nowTime) + a3*math.cos(2*math.pi*4*f3*nowTime);
            # zCurr1 = (a0 + rC1)*math.cos(2*math.pi*f0*zpos) + 0*(a0 + rC1)*math.cos(2*math.pi*f0*zpos + randomComponent1) + (a2 + rC2)*math.cos(2*math.pi*f2*nowTime) + (a2 + rC2)*math.cos(2*math.pi*f2*nowTime + randomComponent2);
            # yCurr1 = random.gauss(0.0,2.0);

                      
            #mild disturbances from overall field
            # xCurr1 = xCurr1 - xCurr
            # yCurr1 = yCurr1 - yCurr
            # zCurr1 = zCurr1 - zCurr
            # xCurrent1,yCurrent1,zCurrent1 = b.vectorFromWorld((xCurr1,yCurr1,zCurr1));
            ...
            # if i == 0:
            #     print i
            # print xdrag1,ydrag1,zdrag1

            # xCurrent1,yCurrent1,zCurrent1 = b.vectorFromWorld((xCurr1,yCurr1,zCurr1));
            # xdrag1 = 0.5*1000*ly/2*lz/2*1.05*(xCurrent1)*math.fabs(xCurrent1)
            # ydrag1 = 0.5*1000*lx/2*lz/2*1.05*(yCurrent1)*math.fabs(yCurrent1)
            # zdrag1 = 0.5*1000*lx/2*ly/2*1.05*(zCurrent1)*math.fabs(zCurrent1)
            # b.addRelForceAtRelPos((xdrag1,ydrag1,zdrag1),pos)
            # print "test drag"
            # print xdrag1,ydrag1,zdrag1
...
def drop_object():
    ...
    # print gx,gy,gz
    # if objcount == 0:
    #     body.setPosition((0.0,0.78,0.0))
    #     theta = 0
    # else:
    #     body.setPosition((0.0,0.25,0.0))
    #     theta = 0
    ...
    for b in bodies:
        ...
        if b.attached == 1:
            ...
            for k in range(0,len(attachedToArray)):
                ...
                #y=bodies[0].getPosition() - bodies[1].getPosition()
                # m=[x0-x1,y0-y1,z0-z1]
                # scalp (m, 3000 / length (m))
                # bodies[attachedToArray[k]-1].addForce(m)
...
def near_callback(args, geom1, geom2):
    ...
    global pullCounter,finishAssembly, formationCount,formationCountE,formationCountF,formationCountG,formationCountD,formationClayerAbove,formationDlayerAbove;
    ...
    if (geom2.shape == "box") and (geom1.shape == "box"): 
        ...
        if len(face1) > 0 and len(face2) > 0: 
            ...
            if (b1.state == 'b' and b2.state == 'a') or (b1.state == 'a' and b2.state == 'b'):
                for i in range(len(face1)):
                    for j in range(len(face2)):
                        if (face1[i] in listOfFaces1 and face2[j] in listOfFaces1): 
                            ...
                            if (b2.id not in b1.attachedTo) and theta > 135.0 and (activatedFace1 not in b1.facesActivated) and (activatedFace2 not in b2.facesActivated):
                                if b1.state == 'a':
                                    b1.state = 'c'
                                    print "faces activated", activatedFace1
                                if b2.state == 'a':
                                    b2.state = 'c'
                                    print "faces activated", activatedFace2
                                ...
                                b2.facesActivated.append(activatedFace2)
                                b2.facesActivatedTo.append(activatedFace1)
                                print "Plus shape being formed"
                                formationCount = formationCount + 1;
                                print formationCount;

            if ((b1.state == 'c' and b2.state == 'a') or (b1.state == 'a' and b2.state == 'c')) and formationCount == 4:
                for i in range(len(face1)):
                    for j in range(len(face2)):
                        if (face1[i] in listOfFaces1 and face2[j] in listOfFaces1): 
                            ...
                            if (b2.id not in b1.attachedTo) and theta > 135.0 and (activatedFace1 not in b1.facesActivated) and (activatedFace2 not in b2.facesActivated):
                                ...
                                if b1.state == 'a':
                                    if len(b1.attachedTo) == 2:
                                        b1.state = 'd';
                                        print "state changed to d"
                                        formationCountD = formationCountD + 1;

                                if b2.state == 'a':
                                    if len(b2.attachedTo) == 2:
                                        b2.state = 'd';
                                        print "state changed to d"
                                        formationCountD = formationCountD + 1;

                                if b1.state == 'c':
                                    if (b1.facesActivated[0] == 3 and activatedFace1 == 4) or (b1.facesActivated[0] == 4 and activatedFace1 == 3):
                                        b2.state = 'e'
                                        print "state changed to e, right left condition 1"
                                        formationCountE = formationCountE + 1;
                                        print "formationCountE chaged to ", formationCountE
                                    if (b1.facesActivated[0] == 5 and activatedFace1 == 6) or (b1.facesActivated[0] == 6 and activatedFace1 == 5):
                                        b2.state = 'e'
                                        print "state changed to e, front back condition 1"
                                        formationCountE = formationCountE + 1;
                                        print "formationCountE chaged to ", formationCountE

                                if b2.state == 'c':
                                    if (b2.facesActivated[0] == 3 and activatedFace2 == 4) or (b2.facesActivated[0] == 4 and activatedFace2 == 3):
                                        b1.state = 'e'
                                        print "state changed to e, right left condition 2"
                                        formationCountE = formationCountE + 1;
                                        print "formationCountE chaged to ", formationCountE
                                    if (b2.facesActivated[0] == 5 and activatedFace2 == 6) or (b2.facesActivated[0] == 6 and activatedFace2 == 5):
                                        b1.state = 'e'
                                        print "state changed to e, front back condition 2"
                                        formationCountE = formationCountE + 1;
                                        print "formationCountE chaged to ", formationCountE
                                ...
                                print "Plus shape being expanded"
                                # os._exit(1);

            if ((b1.state == 'd' and b2.state == 'a') or (b1.state == 'a' and b2.state == 'd')) and formationCountE == 4:
                for i in range(len(face1)):
                    for j in range(len(face2)):
                        if (face1[i] in listOfFaces1 and face2[j] in listOfFaces1): 
                            ...
                            if (b2.id not in b1.attachedTo) and theta > 135.0 and (activatedFace1 not in b1.facesActivated) and (activatedFace2 not in b2.facesActivated):
                                ...
                                b2.facesActivated.append(activatedFace2)
                                b2.facesActivatedTo.append(activatedFace1)
                                print "shape being expanded"
            

            if ((b1.state == 'e' and b2.state == 'f') or (b1.state == 'f' and b2.state == 'e')):
                for i in range(len(face1)):
                    for j in range(len(face2)):
                        if (face1[i] in listOfFaces1 and face2[j] in listOfFaces1): 
                            ...
                            if (b2.id not in b1.attachedTo) and theta > 135.0 and (activatedFace1 not in b1.facesActivated) and (activatedFace2 not in b2.facesActivated):
                                ...
                                b2.facesActivated.append(activatedFace2)
                                b2.facesActivatedTo.append(activatedFace1)
                                print "shape being flattened"
                                formationCountF = formationCountF + 1;

            if ((b1.state == 'f' and b2.state == 'a') or (b1.state == 'a' and b2.state == 'f')) and formationCountF == 8:
                for i in range(len(face1)):
                    for j in range(len(face2)):
                        if (face1[i] in listOfFaces1 and face2[j] in listOfFaces1): 
                            ...
                            if (b2.id not in b1.attachedTo) and theta > 135.0 and (activatedFace1 not in b1.facesActivated) and (activatedFace2 not in b2.facesActivated):
                                ...
                                if b1.state == 'f':
                                    if (b1.facesActivated[0] in rightLeft and activatedFace1 in frontBack) or (b1.facesActivated[0] in frontBack and activatedFace1 in rightLeft):
                                        b2.state = 'g'
                                        print "state changed to g"
                                        b1.attached = 1
                                        b2.attached = 1
                                        b1.attachedTo.append(b2.id)
                                        b2.attachedTo.append(b1.id)
                                        ...
                                        b2.facesActivated.append(activatedFace2)
                                        b2.facesActivatedTo.append(activatedFace1)
                                        formationCountG = formationCountG +1;
                                        
                                    
                                if b2.state == 'f':
                                    if (b2.facesActivated[0] in rightLeft and activatedFace2 in frontBack) or (b2.facesActivated[0] in frontBack and activatedFace2 in rightLeft):
                                        b1.state = 'g'
                                        print "state changed to g"
                                        b1.attached = 1
                                        b2.attached = 1
            ...
            if ((b1.state == 'g' and b2.state == 'f') or (b1.state == 'f' and b2.state == 'g')):
                for i in range(len(face1)):
                    for j in range(len(face2)):
                        if (face1[i] in listOfFaces1 and face2[j] in listOfFaces1): 
            ...
            if ((b1.state == 'b' and b2.state == 'a') or (b1.state == 'a' and b2.state == 'b')) and formationCountD == 4:
                for i in range(len(face1)):
                    for j in range(len(face2)):
            ...
            if (b1.state == 'bprime' and b2.state == 'a') or (b1.state == 'a' and b2.state == 'bprime'):
                for i in range(len(face1)):
                    for j in range(len(face2)):
                        if (face1[i] in listOfFaces1 and face2[j] in listOfFaces1): 
                            ...
                            if (b2.id not in b1.attachedTo) and theta > 135.0 and (activatedFace1 not in b1.facesActivated) and (activatedFace2 not in b2.facesActivated):
                                ...
                                b2.facesActivated.append(activatedFace2)
                                b2.facesActivatedTo.append(activatedFace1)
                                print "above layer plus shape being formed"
                                
                                

            if (b1.state == 'cprime' and b2.state == 'c') or (b1.state == 'c' and b2.state == 'cprime'):
                for i in range(len(face1)):
                    for j in range(len(face2)):
                        if (face1[i] in listOfFaces2 and face2[j] in listOfFaces2): 
                            ...
                            if (b2.id not in b1.attachedTo) and theta > 135.0 and (activatedFace1 not in b1.facesActivated) and (activatedFace2 not in b2.facesActivated):
                                ...
                                b2.facesActivated.append(activatedFace2)
                                b2.facesActivatedTo.append(activatedFace1)
                                print "straightening above layer"
                                formationClayerAbove = formationClayerAbove + 1;

            if ((b1.state == 'd' and b2.state == 'a') or (b1.state == 'a' and b2.state == 'd')):
                for i in range(len(face1)):
                    for j in range(len(face2)):
                        if (face1[i] in listOfFaces2 and face2[j] in listOfFaces2): 
                            ...
                            if (b2.id not in b1.attachedTo) and theta > 135.0 and (activatedFace1 not in b1.facesActivated) and (activatedFace2 not in b2.facesActivated):
                                ...
                                b2.facesActivated.append(activatedFace2)
                                b2.facesActivatedTo.append(activatedFace1)
                                formationDlayerAbove = formationDlayerAbove + 1;

            if ((b1.state == 'bprime' and b2.state == 'a') or (b1.state == 'a' and b2.state == 'bprime')) and formationCountD == 4:
                for i in range(len(face1)):
                    for j in range(len(face2)):
                        if (face1[i] in listOfFaces2 and face2[j] in listOfFaces2): 
                            ...
                            if (b2.id not in b1.attachedTo) and theta > 135.0 and (activatedFace1 not in b1.facesActivated) and (activatedFace2 not in b2.facesActivated):
                                if b1.state == 'a':
                                    b1.state = 'bdoubleprime'
                                    print "icing on the cake"
                                if b2.state == 'a':
                                    b2.state = 'bdoubleprime'
                                    print "top layer finishing"
...
def findFace(pointOfContacts1,pointOfContacts2,geom1,geom2):
    ...
    # print "possible faces geom1"
    # print pointOfContacts1
    # print iter1

    s1 = len(iter1)
    s2 = len(iter2)

    # print "possible faces geom2"
    # print pointOfContacts2
    # print iter2
