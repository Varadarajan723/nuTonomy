function [a,c,n,S] = variogramfitModified(h,gammaexp,a0,c0,numobs,ii,varargin)
...
if params.plotit
    switch lower(type)
        case 'bounded'
            ...
            if (ii==1)
                fplot(@(h) funnugget(b) + func(b,h),[0 b(1)],'b','LineWidth',3)
                hold on
                fplot(@(h) funnugget(b) + b(2),[b(1) max(h)],'b','LineWidth',3)
            else
...
%             plot(h,gammaexp,'rs','MarkerSize',10);
             ...
             if (ii==69)
                ...
                set(findobj(gca, 'Type', 'Line', 'Color', 'b'), 'LineWidth', 5);
    ...
    xlabel('lag distance h','FontSize',20)
    ylabel('\gamma(h)','FontSize',20)
    hold off
end
