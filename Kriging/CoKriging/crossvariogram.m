function S = variogram(x1,y1,x2,y2,varargin)
...
% for crossvariogram approach
iid =  [];
for ii=1:size(x,1)
    for jj = ii:size(x,1)
        distance = calDist(x(ii,:),x(jj,:));
        if distance <= params.maxdist
            iid = [iid;ii jj distance];
        end
    end
end

for ii = 1:size(iid,1)
    if iid(ii,1) <= size(x1,1) && iid(ii,2) <= size(x1,1)
        iid(ii,:) = NaN;
    else
        if iid(ii,1) > size(x1,1) && iid(ii,2) > size(x1,1)
            iid(ii,:) = NaN;
...
function distance = calDist(x1,x2)
    distance = hypot(x1(1,1) - x2(1,1),x1(1,2) - x2(1,2));
end
